...
import { ArrowLeft, FileText, Loader2, User, CheckCircle2, Crop, Sparkles, AlertTriangle, HelpCircle } from "lucide-react";
...
import { AIModeInstructionsDialog } from "@/components/AIModeInstructionsDialog";
...
export default function AIResumeUploader() {
  ...
  const { canUseAIGeneration, usage } = useSubscriptionLimits();
  const [limitReached, setLimitReached] = useState(false);
  const [userHasResumes, setUserHasResumes] = useState(true); // Default to true until we check
  ...
  // Check if user has any resumes and if they've used AI generation before
  useEffect(() => {
    // Check if user has any resumes
    const checkUserResumes = async () => {
      try {
        const response = await fetch('/api/user-resumes/count');
        if (response.ok) {
          const data = await response.json();
          setUserHasResumes(data.count > 0);
        }
      } catch (error) {
        console.error('Failed to check user resumes:', error);
      }
    };
  ...
  // Show instructions if user is new (no resumes + never used AI generation)
  useEffect(() => {
    const aiGenerationCount = usage?.aiGenerationCount || 0;
    const isNewUser = !userHasResumes && aiGenerationCount === 0;
    const hasSeenInstructions = localStorage.getItem('aimode_instructions_seen') === 'true';
    
    // Always show for new users or if not seen before
    setShowInstructionsDialog(isNewUser || !hasSeenInstructions);
  }, [userHasResumes, usage]);

  // Mark that the user has seen the instructions
  const handleInstructionsClose = (open: boolean) => {
    if (!open) {
      localStorage.setItem('aimode_instructions_seen', 'true');
      setShowInstructionsDialog(false);
    }
  };

  // Function to open instructions dialog when help button is clicked
    ...
    <>
      ...
      <div className="min-h-[calc(100vh-64px)] flex">
        <div className="w-full max-w-none p-6 md:p-10">
          {/* Header with Logo, Back Button and Help Button */}
